$('form').submit(function (e) {
  ...
  $.ajax('/', {
    ...
    success: function (response) {
      $('textarea').val('');
      $('.app--tweet--list li:nth-child(5)').remove();
      $('.app--tweet--list').prepend(`
        <li>
          <strong class="app--tweet--timestamp">${response.created_at.slice(0, 10)}</strong>
          <a class="app--tweet--author">
            <div class="app--avatar" style="background-image: url(${response.user.profile_image_url})">
              <img src="images/f-spore.png" />
            </div>
            <h4>${response.user.name}</h4> @${response.user.screen_name}
          </a>
          <p>${response.text}</p>
